...
Widget::Widget(bool resizeEnable,
               ...
               bool winNativeEvent,
               QWidget *parent)
    : QFramelessWidget(parent, resizeEnable, shadowBorder, winNativeEvent), ui(new Ui::Widget)
{
    ui->setupUi(this);
    ...
    // 数据更新信号连接
    connect(analysis, SIGNAL(haveNewData(QVector<QVector<double>>)),
            this, SLOT(haveNewPoint_drawPlot(QVector<QVector<double>>)));
    ...
    // 设置按键指令
    memcpy(keyCmd, configFile.readString("keyCmd").toUtf8().data(), sizeof(char) * 50);
...
void Widget::keyPressEvent(QKeyEvent *e)
{
    ...
    { // 0x41 = A  0x5a = Z 不区分大小写
        for (int i = 0; i < 50; i++)
        {
            int cmdbuff = keyCmd[i];
            if (cmdbuff >= 65 && cmdbuff <= 90)
            { // 符合大写字母区间才进行判断
                if (e->key() == cmdbuff)
                { // 找一下有没有响应的指令，有的话就发送
...
void Widget::findPortTimer_timeout()
{
    ...
    if (listsize != portlist.size())
    {                               // 判断是否改变了设备数量
        ui->comboBox_port->clear(); // 清除之前的显示
        for (int i = 0; i < portlist.size(); i++)
        { // 添加更新后的设备信息
            ui->comboBox_port->addItem(portlist[i].portName() + ":" + portlist[i].description());
        }
    }
...
void Widget::on_checkBox_open_clicked(bool checked)
{
    ...
    else if (ui->comboBox_port->currentIndex() < 0)
    { // 没有搜索到串口
        ui->checkBox_open->setChecked(false);
...
void Widget::serialport_readyread()
{
    ...
    analysis->inputDataStream(data); // 输入到数据解析类中去
...
void Widget::on_spinBox_buff_editingFinished()
{
    ...
    ui->scrollBar_pos->setRange(ui->plotView->getWindSize(), ui->plotView->getBuffSize());
